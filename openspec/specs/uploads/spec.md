...
## Purpose
File storage and folder documents for DataHub: how a file blob is stored and streamed back, how documents are attached to a company's folder, listed, archived, and deleted, and how document activity is recorded. This spec captures the behavior the rebuilt `uploads` module must honor at parity with legacy (generic uploads + folder documents; manual-GL upload sessions are a later phase).
## Requirements
