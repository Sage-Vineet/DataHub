...
### 21. Parity: coverage, refusals, and reconciling two harnesses
- **A collision, handled by keeping the better harness.** `tools/parity` (§19) landed independently while a second harness was being built for the same job. Theirs is better where it counts — it replays against the **same gateway image** with the flag off versus on, isolating exactly what a cutover changes, where the other compared two different origins. So the second harness was retired and its differentiators folded into theirs; two harnesses is worse than either.
- **Refusals, because "staging only" was a config convention rather than an enforced property.** `guards.ts` adds a production-host check and a staging-marker check, both running before any request is issued, and mutating runs now *require* the marker — that is when the harness starts writing. Exit code 3 is distinct from 1 on purpose: "declined to point at that environment" is not "the engines disagreed".
- **Coverage turns the honest caveat into a number.** The report already said a clean run is "evidence for a flag flip, not proof of one". Measuring it against the route surface derived from source: **the declared suite covers 22 of 68 comparable endpoints** — `folders` 3/13, `uploads` 2/9, every domain partial. Before this, 22-of-68 and 68-of-68 both printed "PARITY CLEAN".
- **Second real finding, from the coverage check itself:** `sync-is-deferred-to-legacy` requested `GET /key-reports/chart-of-accounts`, a path **neither** engine serves. Both returned the same 404, so it reported "match" — while its stated purpose is to observe an *expected* difference (legacy answers, module 501). Legacy serves that under `/key-reports/versions/:versionId/...`; repointed. Two other non-comparable scenarios (a module-only endpoint, a deliberate fall-through) are now classified rather than reported as gaps — reporting those as problems is the noise that gets a harness ignored.
- **The artifact seam.** `tools/parity` needs the comparable surface, but deriving it means instantiating every module router, which drags `apps/api`'s Express type augmentation into the importing package — the direct import failed typecheck for exactly that reason. So `apps/api` emits `tools/parity/route-surface.json` and `route-contract.test.ts` fails if it drifts from the live derivation: same guarantee, no coupling.
- **`backend/sql/schema.sql` — nothing had ever applied it, so nothing caught that it cannot work.** Two defects: `bank_transactions` omitted `client_id` while an index referenced it (the running code settles it — `bankVsBooks.js:483` filters on it — so the declaration was stale; fixed), and it references `dataset_versions(id)` without creating that table. `dataset_versions` lives only in the migrations, where it is created **twice with conflicting definitions** (001: `version_number`/`upload_job_id`; 019: `sync_source`/`finalized_by`), both `IF NOT EXISTS`, so which one production has depends on the order they ran. **Production's schema is not recoverable from this repository** — the argument for snapshot-based reconciliation, now demonstrated rather than asserted. The file carries a NOT AUTHORITATIVE banner and `schema-file.test.ts` pins all of it.
- **Drift needs a committed baseline,** not a run log: without one every reconciliation re-derives the same known legacy drift and a new difference is lost in it. `packages/db/src/drift.ts` separates known from fresh and distinguishes *backlog* drift (a column not yet modelled) from *breaking* drift (a column declared that the database lacks — that fails at runtime).
- **Seed-time anonymization, because the hazard is outbound mail.** Staging runs a real Graph emailer, so a reset test mails a real customer from a system nobody watches; the address has to be wrong in the database, not masked in the app. Financial data is deliberately left intact — that trade-off needs the CTO's sign-off before the first snapshot.
- **Verification:** `@datahub/parity` 53 → 73 tests; api 241; db 40; workspace typecheck/lint/test green.
- **Deferred (ops):** snapshot restore, `db:pull`, the first staging run, the rollback drill. Everything they feed is built and tested. `devenv.nix`'s `load-schema` runs the broken schema file — left as an out-of-scope follow-up.
