...
### 19. Cutover platform — parity harness (Phase D)
- **Change:** new workspace package `tools/parity` (`@datahub/parity`). Turns each domain's "enable in staging; parity checklist against the real DB" task into a repeatable run that produces evidence and an exit code.
- **What:** replays the same request against the **same gateway twice** — the domain's flag OFF (legacy) and ON (module) — then normalises, diffs and reports. `normalize.ts` (per-scenario declarations of what may legitimately differ), `diff.ts` (differences classified critical/major/minor), `auth.ts` (authenticates each upstream independently, bearer or cookie — across a cutover they are two different engines), `runner.ts` (concurrent for reads, sequential for writes), `scenarios/` (per-domain suites, each naming the delta-spec requirement it exercises), `cli.ts` (exit 0 clean / 1 differences / 2 could not run).
- **Design stance — noise control is the whole game.** Ids, timestamps and unordered rows differ between any two implementations; a harness that reports them gets ignored, which is worse than not having one. So `volatile` masks a *value* while still comparing the *key* (a field that vanishes is still caught), `sortArraysBy` is opt-in per path (where order is contractual, a reordering is reported), and rules are narrow by design so widening one is a reviewable act. Mutating scenarios are skipped unless asked for — they write to **both** upstreams.
- **First finding, before it was ever pointed at a running stack:** the SPA sends `?includeArchived=true` and legacy reads `req.query.includeArchived`, but the `folders` and `uploads` modules read only `include_archived`. After cutover the parameter is simply absent — the query parses cleanly and the filter never applies, so the archived view silently returns the unfiltered list (`fileExplorerStore.js` requests the tree with `includeArchived: true`). **Unit and integration tests missed it because they asked using the module's own parameter name**: the coverage was real, the contract was not. Fixed in both contracts and routers, with tests pinning the wire name; reverting the contract fails the new integration test.
- **Verification:** 53 tests, 91.6% coverage. `runner.test.ts` drives the harness end-to-end against two real Express servers, asserting it **fails** on a difference that matters and **passes** on ids and ordering that legitimately differ, plus login failure, unreachable upstream, and absolute-expectation cases. The CLI was run against live upstreams reproducing the defect: it reported `[major] value at $.length — legacy: 2, candidate: 1` against the right spec, and exited 1.
- **Caveat carried in the output itself:** a clean run is evidence for a flag flip, not proof of parity — it covers the declared scenarios against the data currently seeded.

### 20. Activity log capture — the first product capability built new (SE-0004)
- **Why:** `SE - 0004` is the only capability in the product surface whose value *expires*. It is a capture problem: activity not recorded when it happened is gone, and `BR - 0010` (buyer engagement analytics, described in the product list as a key demo feature) is built entirely from history. It also has **no legacy equivalent** — nothing to cut over, no parity to prove, no handler to delete — so it could be built greenfield while the cutover work proceeded independently.
- **The design problem, and what it forced:** most traffic is still legacy for the whole of Phase C. A log that only saw the new stack would miss almost everything — exactly the gap the capability exists to close. So capture is **two-tier**: a request envelope written at the gateway for *every* request (legacy-proxied included, attributed by a `proxyReq` hook), plus typed semantic events emitted by migrated modules, joined by correlation id. Tier 1 covers everything and understands nothing; tier 2 understands its domain and sees a minority of traffic. Neither alone is sufficient.
- **Constraints the existing code imposed:** the gateway runs no body parser so uploads stream through (`gateway.ts`, and `shared/router.ts` exists because `router.use()` previously consumed request streams). Capture therefore attaches to the response `finish` event and is envelope-only — a supertest asserts a 200KB POST body reaches legacy byte-identical with capture enabled, guarding that regression class directly.
- **Immutability enforced twice, not asserted once:** the repository exposes `append` and reads with no update/delete method at all, *and* the application's database role holds INSERT+SELECT but not UPDATE/DELETE. Both are tested — the grant test runs `SET ROLE` under PGlite and asserts UPDATE and DELETE are refused. Tamper evidence is a per-record hash chain (content hash + previous hash) with a verification pass; documented as consistency evidence, **not** proof against a privileged attacker who can rewrite the chain.
- **Honest degradation:** the writer is async with a bounded buffer, so capture never blocks or fails a request. On overflow or write failure it records a **gap marker** — interval, count, reason — rather than dropping silently, and holds the marker pending until storage recovers. A quiet period with no marker means nothing happened; with one, it means capture was shedding.
- **Partitioned monthly from day one**, with a DEFAULT partition always present so a month nobody rolled forward lands rather than erroring — a misfiled audit record is recoverable, a dropped one is not.
- **Verification:** api **179 → 232 tests**; `src/activity` at 95.9% statements. Emission is tested against the *real* folders and uploads routers, not stand-ins, and asserts events fire only **after** the authorization guard — writing that test caught a real ordering bug where `document.opened` was emitted before the access check and would have recorded opens that were actually denied.
- **Coverage limits are written down** in `docs/ACTIVITY_LOG_COVERAGE.md`: view duration and print wait on the `DR - 0006` viewer, add-back/valuation/signature events wait on modules that do not exist, and legacy-served requests have envelopes but no semantic events until their domain flips.
- **Not in this change:** the read surface — search, filter, export, alerting, and the log's own access control. All buildable over history later; capture is not.

### 20. Staging parity harness — making a flip a decision instead of a leap
- **Why:** nine domains built, none serving a request, `auth-production-cutover` 0/23, and no legacy handler ever deleted. §18 proved the cutover *mechanism* under compose with a hand-run checklist; what was missing was an environment with production-shaped data and a repeatable way to show the module answers a request the way legacy does. Every domain change was writing its own prose parity checklist — which does not scale to nine domains, is not re-runnable, and produces no artifact to point at when authorizing the one irreversible step in the program.
- **The request set is derived from source, never hand-listed.** `route-contract.test.ts`'s legacy/module surface derivation moved into `parity/routes.ts`, shared by the guard and the harness, and split three ways: compare (both serve it), additive (module-only), backlog (legacy-only). Adding a route to a module now automatically widens what parity must prove. A hand-listed set is the failure mode this exists to prevent — it stays green while the surface underneath it moves.
- **Comparison is status + shape + declared invariants,** not bytes: ids and timestamps collapse to type placeholders so a differing uuid is not a difference, while a missing key fails with its field path. Shape equality is weak evidence on its own — two lists of different lengths have identical shapes — so per-endpoint invariants are the layer that actually carries confidence. Latency is recorded and never gates.
- **Two independent refusals before any request is issued:** the target host must not be on `PARITY_PRODUCTION_HOSTS` (an unparseable connection string is refused too — "I could not tell" is not "it is safe"), and the target must report the marker the seed writes. Tests assert zero outbound requests on each refusal. Exit code 3 is deliberately distinct from 1: "declined to point at that database" is not "the engines disagreed".
- **Coverage travels with every verdict.** A report showing twelve green endpoints and silent about the thirty it never touched reads as "parity proven" when it means "parity sampled" — and that misreading is what would authorize deleting a handler too early. Every renderer prints compared-of-total, names each skip reason, and a partial run prints an explicit "this run SAMPLED the surface" warning. `docs/CUTOVER_FLIP_CRITERIA.md` separates what authorizes a **flip** (reversible) from what authorizes a **deletion** (not).
- **Seed-time anonymization, because the real hazard is outbound mail.** Staging runs a real Graph emailer, so a reset test mails a real customer from a system nobody is watching; masking in the app layer does not prevent that, the address has to be wrong in the database. Emails are rewritten to a sink address encoding the row id, phones to the reserved 555-01xx range, and financial data is deliberately left intact — that trade-off needs the CTO's explicit sign-off before the first snapshot (design Q3).
- **`backend/sql/schema.sql`: nothing ever applied it, so nothing caught that it cannot work.** Applying it to an empty database found two defects. (1) `bank_transactions` omitted `client_id` while an index referenced it — the running code settles it (`bankVsBooks.js:483` filters on `client_id`), so the *declaration* was stale; fixed. (2) It references `dataset_versions(id)` and never creates that table — it lives only in the migration set, where it is created **twice with conflicting definitions** (001 has `version_number`/`upload_job_id`; 019 has `sync_source`/`finalized_by`), both under `CREATE TABLE IF NOT EXISTS`, so which one production has depends on the order they ran in. **Production's actual schema is therefore not recoverable from this repository** — which is precisely the argument for snapshot-based reconciliation, now demonstrated rather than asserted. The file carries a NOT AUTHORITATIVE banner and `schema-file.test.ts` pins all of it.
- **Drift needs a committed baseline,** not a run log: without one, every reconciliation re-derives the same known legacy drift and a genuinely new difference is lost in it. `packages/db/src/drift.ts` separates known from fresh, and distinguishes *backlog* drift (a column we have not modelled) from *breaking* drift (a column we declared that the database lacks — that one fails at runtime).
- **Verification:** api **232 → 293 tests**, db **26 → 40**; workspace typecheck/lint/test green. `route-contract.test.ts` passes unchanged against the extracted derivation.
- **Deferred, and why:** the snapshot restore, `db:pull`, the first staging run and the rollback drill all need an environment that does not exist yet (ops). Everything they feed — the seed, the reconciler, the harness, the report — is built and tested. `devenv.nix`'s `load-schema` script runs the broken schema file and is left alone as an out-of-scope follow-up.
