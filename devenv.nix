...
in
{
  ...
  # Bootstrap a dev database.
  #
  # One command, with a recorded history and a checksum guard
  # (packages/db/scripts/migrate.mjs). It used to be three, the first of which
  # was a tolerant load of `backend/sql/schema.sql` — a file that could not do
  # the job its name claimed, ending with 14 statements indexing tables it never
  # created, and carrying none of the DDL the modernized modules needed. There
  # was no authoritative source for the schema, so the script counted the
  # failures instead of fixing them.
  #
  # `0000_baseline.sql` is that authoritative source.
