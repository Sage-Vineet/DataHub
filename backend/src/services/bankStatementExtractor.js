...
async function extractBankStatementsFromPdfBase64(pdfBase64, fileName = "bank_statement.pdf") {
  ...
  if (hasText) {
    ...
    console.log(`[BankPDF] Gemini text returned [] for "${fileName}", trying multimodal Gemini`);
  }

  // Final fallback: multimodal Gemini (kept last so text-layer PDFs exhaust deterministic parsing first)
  try {
    const geminiResults = await extractViaGemini(pdfBase64, fileName);
    if (geminiResults.length > 0) return geminiResults;
    console.log(`[BankPDF] Gemini returned [] for "${fileName}"`);
  } catch (err) {
    console.warn(`[BankPDF] Gemini PDF failed for "${fileName}": ${err.message}`);
  }
