...
function isStructureLockedFolder(item) {
  return isLockedFolder(item);
}

function canAddFoldersHere(item) {
  if (!item) return false;
  if (item.id === 'root') return true;
  return canCreateChildFolders(item);
}
...
function FolderTreeNode({ node, depth = 0, canAccessFolder }) {
  ...
  const canAcceptFolderDrop = canAddFoldersHere(node);
...
function FolderTree({ tree, onUpload, role, getFolderPermissions }) {
  ...
  const canCreateFolderHere = canWrite && canAddFoldersHere(currentFolder);
    ...
    <aside className="w-60 flex-shrink-0 bg-white border-r border-gray-100 flex flex-col overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-100">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => { if (canCreateFolderHere) startNewFolder(currentFolderId); }}
            disabled={!canCreateFolderHere}
            className={`flex-1 flex items-center justify-center gap-1 px-2 py-1.5 rounded-lg text-xs font-semibold transition-colors border ${canCreateFolderHere ? 'bg-[#05164D]/8 text-[#05164D] hover:bg-[#05164D]/15 border-[#05164D]/15' : 'bg-gray-100 text-[#A5A5A5] border-gray-200 cursor-not-allowed'}`}
          >
...
function TopBar({ tree, currentPath, onUpload, role, currentFolderPermissions }) {
  ...
  const canCreateFolderHere = canWrite && canAddFoldersHere(currentFolder);
  const selectionHasLockedFolder = selectedItems.some((id) => {
    const item = findById(tree, id);
    return isStructureLockedFolder(item);
  });
    ...
    <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 flex-wrap flex-shrink-0">
      ...
      {selectedItems.length > 0 && role === 'broker' && (
        <div className="flex items-center gap-1 px-2 py-1 bg-[#05164D]/8 rounded-xl border border-[#05164D]/15">
          ...
          <button
            onClick={() => { if (!selectionHasLockedFolder) deleteItems(selectedItems); }}
            disabled={selectionHasLockedFolder}
            className={`ml-2 p-1 rounded-lg transition-colors ${selectionHasLockedFolder ? 'text-[#C0C4CC] cursor-not-allowed' : 'hover:bg-red-50 text-red-500'}`}
      ...
      <button
        onClick={() => { if (canCreateFolderHere) startNewFolder(currentFolderId); }}
        disabled={!canCreateFolderHere}
        className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-semibold transition-colors ${canCreateFolderHere ? 'border-gray-200 text-[#6D6E71] hover:bg-gray-50' : 'border-gray-200 text-[#A5A5A5] bg-gray-100 cursor-not-allowed'}`}
      >
...
function FileCard({ item, role, permissions, sharedMeta, onShareAccess, onMoveFolder }) {
  ...
  const isLocked = item.type === 'folder' && isStructureLockedFolder(item);
  const canManageStructure = canManage && !isLocked;
  ...
  const handleDragOver = (e) => {
    if (!canManage || item.type !== 'folder' || !canAddFoldersHere(item)) return;
    e.preventDefault();
    e.stopPropagation();
    setDragOver(item.id);
  };
  ...
  const handleDrop = (e) => {
    ...
    e.stopPropagation();
    if (!canManage) return;
    if (item.type === 'folder' && draggingItems.length > 0 && canAddFoldersHere(item)) {
      ...
      {item.type === 'folder' && canManage && (
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <FolderActionMenu
            ...
            onRename={canManageStructure ? () => startRenaming(item.id) : null}
            onMove={canManageStructure ? () => onMoveFolder(item) : null}
            onDelete={canManageStructure ? () => useFileExplorerStore.getState().deleteItems([item.id]) : null}
          />
        </div>
      )}
...
function FileGrid({
  ...
  return (
    <div className="flex flex-wrap gap-3 p-1 content-start">
      {newFolderParentId === currentFolderId && (role === 'broker' || currentFolderPermissions.write) && canAddFoldersHere(currentFolder) && (
...
function ContextMenu({ tree }) {
  ...
  const ids = selectedItems.length > 1 && selectedItems.includes(contextMenu.itemId)
    ? selectedItems
    : [contextMenu.itemId];
  const hasLockedFolderSelection = ids.some((id) => {
    const selectedItem = findById(storeTree, id);
    return isStructureLockedFolder(selectedItem);
  });
...
// ── EmptyState ─────────────────────────────────────────────────────────────────
function EmptyState({ currentFolderId, currentFolder, onUpload, canWrite }) {
    ...
    <div className="flex-1 flex flex-col items-center justify-center py-20 text-center">
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={() => { if (canCreateFolderHere) startNewFolder(currentFolderId); }}
          disabled={!canCreateFolderHere}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors border ${canCreateFolderHere ? 'border-gray-200 text-[#6D6E71] hover:bg-gray-50' : 'border-gray-200 text-[#A5A5A5] bg-gray-100 cursor-not-allowed'}`}
        >
...
export default function FileExplorer({ role = 'broker', title, companyId, currentUserId }) {
  ...
  useEffect(() => {
    const handle = (e) => {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && selectedItems.length > 0 &&
        ...
        if (role === 'broker' && !selectionHasLockedFolder) {
          useFileExplorerStore.getState().deleteItems(selectedItems);
        }
      }
      if (e.key === 'F2' && selectedItems.length === 1) {
        if (role === 'broker' && !selectionHasLockedFolder) {
          useFileExplorerStore.getState().startRenaming(selectedItems[0]);
        }
      }
    };
    window.addEventListener('keydown', handle);
    return () => window.removeEventListener('keydown', handle);
  }, [selectedItems, clearSelection, hideContextMenu, role, tree]);
  ...
  const openMoveFolder = (folder) => {
    if (!canManageAccess || folder?.type !== 'folder' || isStructureLockedFolder(folder)) return;
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        >
          ...
          {currentItems.length === 0 && newFolderParentId !== currentFolderId ? (
            <EmptyState currentFolderId={currentFolderId} currentFolder={currentFolder} onUpload={openUpload} canWrite={canWriteCurrent} />
          ) : view === 'grid' ? (
            <FileGrid
              items={currentItems}
