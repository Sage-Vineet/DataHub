...
// ── Full 15-level DISPLAY hierarchy (presentation only — never stored) ────────
// Builds a complete Level 1–15 view for the grid + export, per the client spec:
//   L1  = Financial Statement (Balance Sheet / Profit & Loss)
//   L2  = Major Section (Assets / Liabilities / Equity / Revenue / COGS / Operating Expenses)
//   L3… = the meaningful intermediate categories from the stored hierarchy
//   gaps = inherit the nearest parent (repeat it down) so no level is ever blank
//   L15 = the original GL account name
// The stored chart_of_accounts rows are NOT modified — the financial reports keep
// reading the true (unpadded) hierarchy; this only shapes what's shown/exported.
const FIN_STMT_LABEL = { balance_sheet: "Balance Sheet", profit_loss: "Profit & Loss" };
const MAJOR_SECTION_LABEL = {
  asset: "Assets", liability: "Liabilities", equity: "Equity",
  income: "Revenue", cogs: "Cost of Goods Sold", expense: "Operating Expenses",
};
// Section-root anchors the stored hierarchy starts with (Total Revenue, Total
// Assets, …) — dropped here because L2 already carries the major section.
const ROOT_ANCHOR_RE = /^total\s+(revenue|income|expenses?|cost of goods sold|cost of sales|assets?|liabilit(?:y|ies)|equity)$/i;

function buildDisplayLevels(row) {
  const acctName = String(row.sourceName || row.accountName || row.adjustedName || "").trim();
  const stmt  = FIN_STMT_LABEL[row.statementType] || "";
  const major = MAJOR_SECTION_LABEL[row.accountType] || "";

  // Meaningful intermediate categories: drop blanks, collapse consecutive
  // repeats, then drop the trailing leaf (the account itself), the leading
  // section-root anchor(s), and any leading label equal to the major section.
  let mids = (row.levels || []).map((l) => (l ? String(l).trim() : "")).filter(Boolean);
  const dd = [];
  for (const l of mids) if (!dd.length || dd[dd.length - 1] !== l) dd.push(l);
  mids = dd;
  if (mids.length) mids = mids.slice(0, -1);                 // drop leaf
  while (mids.length && ROOT_ANCHOR_RE.test(mids[0])) mids.shift();
  while (mids.length && major && mids[0].toLowerCase() === major.toLowerCase()) mids.shift();

  const out = new Array(MAX_LEVELS).fill("");
  out[0] = stmt || major || acctName;                       // L1
  out[1] = major || stmt || acctName;                       // L2
  let i = 2;
  for (const m of mids) { if (i >= MAX_LEVELS - 1) break; out[i] = m; i += 1; } // L3…
  for (; i < MAX_LEVELS - 1; i += 1) out[i] = out[i - 1];    // inherit parent into gaps
  out[MAX_LEVELS - 1] = acctName || out[MAX_LEVELS - 2] || ""; // L15 = GL account name
  return out;
}

// Human-readable path from a display-levels array (collapse the inherited repeats).
function displayPath(levelsArr) {
  const dd = [];
  for (const l of levelsArr) if (l && (!dd.length || dd[dd.length - 1] !== l)) dd.push(l);
  return dd.join(" > ");
}
    ...
    for (const section of [...SECTION_DEFS, NEEDS_MAPPING_SECTION]) {
      ...
      for (const sg of section.subGroups) {
        ...
        for (const row of allGrouped[sg.key] || []) {
          const lvls = buildDisplayLevels(row);
          sheetRows.push([
            row.systemId || "",
            row.accountNumber || "",
            row.sourceName || "",
          ...
          <table className="min-w-max w-full border-collapse text-sm">
            ...
            <tbody>
              {tableRows.map((item) => {
                ...
                // Full 15-level display view (L1=statement, L2=section, …,
                // L15=GL account name; gaps inherit the parent). Presentation
                // only — the stored hierarchy is unchanged. See buildDisplayLevels.
                const levels    = buildDisplayLevels(row);
                const hierPath  = displayPath(levels);
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      ...
                      title={hierPath}
                    >
                      <span className="block max-w-[220px] truncate">{hierPath || "—"}</span>
                    </td>
