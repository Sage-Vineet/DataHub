 ...
 *   - kpis[]        → array of KPI card objects with icons (same shape as reportService)
...
/**
 * Build icon-annotated KPI card array from a raw kpis object returned by the backend.
 */
 ...
 */
export async function loadQMSDashboard(selectedYear = "all", options = {}) {
  const payload = await getQMSDashboard(options);

  // years[] from backend is ["All Files", "2025", "2024", ...]
  ...
  // Trends: backend returns [{ year, revenue, expenses, netProfit }] sorted ascending
  // Map to chart format [{ name, fullLabel, revenue, expenses }]
  ...
  return {
    ...
    // Pass through the full payload so callers can access years[] for the dropdown
    _raw: payload,
  };
}
