...
export function extractEbitdaFromManualPLRows(rows, asOfDate) {
  ...
  const addBackRows = [
    ...interestExpense.items, ...taxes.items, ...depreciation.items, ...amortization.items,
    ...officerWages.items, ...officerPayrollTax.items, ...realEstateTax.items, ...gainLossAssets.items,
  ];
  const uniqueAddBackKeys = new Set();
  let uniqueAddBackTotal = 0;
  addBackRows.forEach((row) => {
    const key = `${normalize(row.label)}:${row.value}`;
    if (!uniqueAddBackKeys.has(key)) { uniqueAddBackKeys.add(key); uniqueAddBackTotal += row.value; }
  });
  ...
  return {
    ...
    reportPeriod: { startDate: asOfDate || "", endDate: asOfDate || "", reportBasis: "Manual Upload", currency: "USD", time: "" },
    hasData: flatRows.length > 0,
    _debug: { totalFlatRows: flatRows.length, topLevelRows: rows?.length || 0, uniqueAddBackTotal, flatRows },
  };
}
