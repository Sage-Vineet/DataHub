...
export default function WorkspaceReports() {
  ...
  const [isDeliveringExport, setIsDeliveringExport] = useState(false);
  ...
  const reportExportConfig = useMemo(() => {
    if (selectedTab === "Balance Sheet") {
      return {
        reportKey: "balancesheet",
        folderPath: REPORT_FOLDER_PATHS.balancesheet,
      };
    }
    if (selectedTab === "Profit & Loss") {
      return {
        reportKey: "profit-loss",
        folderPath: REPORT_FOLDER_PATHS["profit-loss"],
      };
    }
    return {
      reportKey: "cashflow",
      folderPath: REPORT_FOLDER_PATHS.cashflow,
    };
  }, [selectedTab]);

  const buildCurrentReportExport = async (format) => {
    ...
    if (isEmpty) {
      throw new Error("No active report data found to export.");
    }

    const subtitle = `Report Period: ${appliedStartDate || "N/A"} to ${appliedEndDate || "N/A"} | ${appliedAccountingMethod} Basis`;
    const extension = format === "PDF" ? "pdf" : "xlsx";
    const fileName = buildReportFileName({
      reportKey: reportExportConfig.reportKey,
      extension,
      accountingMethod: appliedAccountingMethod,
      startDate: appliedStartDate,
      endDate: appliedEndDate,
      variant: appliedReportType === "Detail" ? "detail" : "",
    });

    let blob = null;

    if (format === "PDF") {
      blob = await createPdfBlob("report-content");
    } else if (appliedReportType === "Summary") {
      blob = createExcelBlob(
        selectedTab,
        subtitle,
    ...
    if (!blob) {
      throw new Error(`Could not generate ${format} report.`);
    }

    return {
      blob,
      fileName,
      folderPath: reportExportConfig.folderPath,
    };
  };

  const openExportDestinationModal = () => {
    setPendingExport({ format: reportFormat });
  };

  const handleExportDestinationSelect = async (destination) => {
    if (!pendingExport) return;

    const format = pendingExport.format;
    setIsDeliveringExport(true);
    if (format === "PDF") {
    ...
    try {
      const { blob, fileName, folderPath } = await buildCurrentReportExport(format);
      ...
      showToast({
        type: "error",
        title: `Could not export ${format}`,
        message: error.message || "Please try again.",
        duration: 4500,
      });
    } finally {
      setIsDeliveringExport(false);
  ...
  return (
    <div className="page-container">
        ...
        <div className="card-base card-p min-h-[800px] flex flex-col">
          ...
          <div className="mb-8 flex flex-wrap items-center gap-6 border-b border-border-light pb-6">
            ...
            <div className="ml-auto flex items-end gap-3 self-end">
              ...
              {reportFormat === "Excel" ? (
                <button
                  onClick={openExportDestinationModal}
                  disabled={isDownloading || isLoading || isDeliveringExport}
                ...
                <button
                  onClick={openExportDestinationModal}
                  disabled={isDownloadingPDF || isLoading || isDeliveringExport}
                  className="btn-primary h-9 px-4 shadow-sm"
                >
                  {isDownloadingPDF ? (
                    <RefreshCw size={16} className="animate-spin" />
      ...
      <ExportDestinationModal
        ...
        title={`${selectedTabLabel} ${pendingExport?.format || reportFormat}`}
        description="Choose whether to download this export locally or upload it into the matching DataRoom folder."
        isSubmitting={isDeliveringExport}
        onClose={() => {
          if (!isDeliveringExport) setPendingExport(null);
        }}
        onSelect={handleExportDestinationSelect}
      />
    </div>
  );
}
