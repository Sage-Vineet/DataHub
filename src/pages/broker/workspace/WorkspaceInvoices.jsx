...
// ✅ Hook defined as a normal function — uses the module-level cache above
function useInvoices() {
  const [invoices, setInvoices] = useState(() => invoiceCache.data ?? []);
  const [isLoading, setIsLoading] = useState(!invoiceCache.data);
  const [error, setError] = useState("");

  useEffect(() => {
    const isFresh =
      invoiceCache.data &&
      invoiceCache.timestamp &&
      Date.now() - invoiceCache.timestamp < invoiceCache.TTL_MS;
    ...
    async function loadInvoices() {
      ...
      try {
        ...
        invoiceCache.data = normalized;
        invoiceCache.timestamp = Date.now();

        if (isMounted) setInvoices(normalized);
      } catch (err) {
        if (isMounted) {
  ...
  const invalidateCache = () => {
    invoiceCache.data = null;
    invoiceCache.timestamp = null;
  };

  return { invoices, setInvoices, isLoading, error, invalidateCache };
}
...
export default function WorkspaceInvoices() {
  ...
  // ✅ Hook called inside the component, with invalidateCache destructured
  const { invoices, setInvoices, isLoading, error, invalidateCache } =
    useInvoices();
  ...
  // ✅ handleSync inside the component, calls invalidateCache before reload
  ...
  const getInvoiceExportDateRange = () => {
    const now = new Date();
  ...
  const buildInvoiceExport = () => {
    if (!monthRows.length) {
      throw new Error("No invoice summary data found to export.");
    }

    const { startDate, endDate } = getInvoiceExportDateRange();
    const rows = monthRows.map((row) => ({
      Month: row.monthLabel,
      Year: row.year,
      "Total Posted Amt": row.totalPostedAmount,
      "Service %": `${row.servicePercent.toFixed(2)}%`,
      "Invoice Amount": row.invoiceAmount,
      "Total EV": row.totalEV,
      "$ per EV": row.dollarsPerEV,
      "Total PA": row.totalPA,
      "$ per PA": row.dollarsPerPA,
      Notes: row.notes,
      "Discount ($)": row.discountValue,
      "Client Final Total": row.clientFinalTotal,
    }));

    const subtitle = `Invoice Summary | Year ${selectedYear} | Status: ${statusFilter} | Date Filter: ${dateFilter}`;
    const blob = createExcelBlob("Invoices", subtitle, rows);
    if (!blob) {
      throw new Error("Could not generate invoice export.");
    }
    ...
    return {
      ...
      folderPath: REPORT_FOLDER_PATHS.invoices,
    };
  };

  const handleInvoiceExportDestination = async (destination) => {
    setIsExporting(true);

    try {
      const { blob, fileName, folderPath } = buildInvoiceExport();
      ...
      console.error("Invoice export failed:", exportError);
      showToast({
        type: "error",
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#8BC53D] px-4 py-3 text-[14px] font-semibold text-white shadow-[0_12px_24px_rgba(139,197,61,0.28)] transition-all hover:bg-[#78AA32] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isExporting ? (
      ...
      <ExportDestinationModal
        ...
        title="Invoices Excel"
        description="Choose whether to download the invoice summary locally or upload it to the Invoices folder in DataRoom."
        isSubmitting={isExporting}
        onClose={() => {
          if (!isExporting) setShowExportDestinationModal(false);
        }}
        onSelect={handleInvoiceExportDestination}
      />
    </>
  );
}
