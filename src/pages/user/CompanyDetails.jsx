...
export default function UserCompanyDetails() {
  ...
  useEffect(() => {
    ...
    const fetchCompanyData = async () => {
      try {
        const [details, companies, allRequests, folders] = await Promise.all([
        ...
        const requests = allRequests.filter((request) => isOwnedByUser(request, user?.id));
  ...
  }, [clientId, assignedCompany, user?.id]);
